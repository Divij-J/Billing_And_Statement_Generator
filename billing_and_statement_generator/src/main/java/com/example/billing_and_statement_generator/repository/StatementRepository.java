...
public interface StatementRepository extends JpaRepository<Statement, UUID>{
    ...
    @Query("SELECT s FROM Statement s WHERE s.billingCycle.cycleId = :cycleId")
    Optional<Statement> findByCycleId(@Param("cycleId") UUID cycleId);

    //Find all statements for a specific card
    @Query("SELECT s FROM Statement s WHERE s.card.cardId = :cardId ORDER BY s.statementDate Desc")
    List<Statement> findByCard_Card_idOrderByStatementDateDesc(UUID cardId);

    //Check if statement already exists before generating
    @Query("SELECT COUNT(s) > 0 FROM Statement s WHERE s.billingCycle.cycleId = :cycleId")
    ...
    @Query("SELECT s FROM Statement s WHERE s.card.cardId = :cardId AND s.statementStatus = :status")
